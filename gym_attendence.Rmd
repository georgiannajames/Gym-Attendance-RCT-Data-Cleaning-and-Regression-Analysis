...
```{r include=FALSE}
true_data <- read_csv(here("data", "hoursatgymRCT.csv"))

pset1_data <- read_csv(here("data", "hoursatgymRCT.csv"))
...
regression <- lm(hours~treatment, data = true_data)
...
```{r include=FALSE}
pset1_data <- pset1_data %>% 
  replace_with_na(age == -99)
```

```{r include=FALSE}
summary(pset1_data$age)
```
Changing the -99 age participants to look at the distribution of age is helpful in understanding the population of the study, as well as the relationship between age and hours. When you compare the hour distribution of the previously age =-99 to the rest of the distribution, they look pretty similar, which supports my assumptions to change these to the mean. 
```{r echo=TRUE}
...
```{r include=FALSE}
summary(pset1_data$bmi)
```
BMI typically falls between 15% and 30%
...
summary(true_data$bmi)

```


Because these outliers form a similar shaped distribution to the rest of the bmi data, I assume all the recorded BMI values that are less than 1 were actually bmi's reported as percentages, so I changed these from decimal percentages to numbers by multiplying by 100. For example 0.22 becomes 22. 

## Education cleaning

```{r include=FALSE}
unique(pset1_data$education)
```
We have three variables for education, and only three variables were reported! 


## Race_ethnicity cleaning
```{r include=FALSE}
unique(pset1_data$race_ethnicity)
```
...
```{r echo=TRUE}
regression <- lm(hours~treatment, data = pset1_data)
